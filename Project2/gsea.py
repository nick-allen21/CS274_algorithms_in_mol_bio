...
class GSEA:
    ...
    def get_enrichment_score(self, geneset):
        ...
        # S' used in the ES walk (overlap only)
        S_prime = S & set(L)
        G = len(S_prime)
        if G == 0 or G == N:
            return 0.0
        ...
        for gene in L: 
            if gene in S_prime:
                # sqrt weighting uses the OVERLAP size
...
def main():
    ...
    # run GSEA methd
    gsea = GSEA()
    significant_sets = gsea.run_gsea(expfile, sampfile, keggfile)
